...
public class VerifyController : ControllerBase
{
    ...
    public async Task<IActionResult> Verify([FromBody] VerifyRequest? req)
    {
        ...
        if (sha256Match)
        {
            ...
            // ImageDup=true means the claim is fraudulent — AmountVerified is irrelevant.
            _logger.LogInformation("verify: L1 dup sha256={Hash} — fast reject", sha256[..8]);
            return Ok(new VerifyResponse { AmountVerified = false, ImageDup = true });
        }
        ...
        _logger.LogInformation(
            "verify: sha256={Hash} amountVerified={Av} imageDup={Dup} source={Src}",
            sha256[..8], amountVerified, imageDup, intel.AmountSource);
